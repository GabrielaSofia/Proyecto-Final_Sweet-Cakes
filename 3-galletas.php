		...
		if(count($results)==0){
			...
			if(!empty($nom_cliente) && !empty($ced_cliente) && !empty($tel_cliente) && !empty($dir_cliente)){
				...
				if(!empty($sabor) && !empty($tamaño) && !empty($cantidad) && !empty($decoracion) && !empty($fecha_pedido) && !empty($fecha_entrega) && !empty($diseño) && !empty($abono) && !empty($preciototal)){
					$sql = $conexion->prepare("INSERT INTO registro_galletas (ced_cliente,estado,sabor,tamano,cantidad,decoracion,fecha_pedido,fecha_entrega,diseno,descripcion,abono,total,saldo)
									VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
					...
					if($sql->execute()){
						...
						window.location='3-registrar_galletas.php'; </script>";
					}else{
